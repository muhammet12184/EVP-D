...
class PIDRouter:
    ...
    def _load_database(self):
        ...
        if not os.path.exists(self.csv_file):
            # EV unified professional CSV'yi dene
            ev_csv = "ev_unified_professional.csv"
            if os.path.exists(ev_csv):
                self.csv_file = ev_csv
            else:
            ...
            for row in reader:
                ...
                if name.startswith('==='):
                    ...
                    if 'MANUFACTURER SPECIFIC' in name.upper():
                        ...
                        # EV unified professional CSV formatında marka tespiti
                        name_upper = name.upper()
                        if 'NISSAN' in name_upper or 'LEAF' in name_upper or 'RENAULT' in name_upper or 'ZOE' in name_upper:
                            current_manufacturer = 'NISSAN/RENAULT'
                        elif 'HYUNDAI' in name_upper or 'KIA' in name_upper or 'KONA' in name_upper or 'IONIQ' in name_upper or 'NIRO' in name_upper or 'EV6' in name_upper:
                            current_manufacturer = 'HYUNDAI/KIA'
                        elif 'PSA' in name_upper or 'PEUGEOT' in name_upper or 'OPEL' in name_upper or 'CITROEN' in name_upper:
                            current_manufacturer = 'PSA'
                        elif 'BMW' in name_upper or 'I3' in name_upper or 'I4' in name_upper or 'IX' in name_upper:
                            current_manufacturer = 'BMW'
                        elif 'MERCEDES' in name_upper or 'EQ' in name_upper or 'EQA' in name_upper or 'EQB' in name_upper or 'EQE' in name_upper or 'EQS' in name_upper:
                            current_manufacturer = 'MERCEDES'
                        elif 'AUDI' in name_upper or 'E-TRON' in name_upper or 'Q4' in name_upper or 'Q8' in name_upper or 'GT' in name_upper:
                            current_manufacturer = 'AUDI'
                        elif 'VOLVO' in name_upper or 'EX30' in name_upper or 'C40' in name_upper or 'XC40' in name_upper or 'RECHARGE' in name_upper:
                            current_manufacturer = 'VOLVO'
                        elif 'PORSCHE' in name_upper or 'TAYCAN' in name_upper:
                            current_manufacturer = 'PORSCHE'
                        elif 'BYD' in name_upper or 'ATTO' in name_upper or 'DOLPHIN' in name_upper or 'SEAL' in name_upper:
                            current_manufacturer = 'BYD'
                        elif 'MG' in name_upper or 'ZS EV' in name_upper or 'MG4' in name_upper:
                            current_manufacturer = 'MG'
                        elif 'TESLA' in name_upper or 'MODEL' in name_upper:
                            current_manufacturer = 'TESLA'
                        elif 'TOGG' in name_upper or 'T10X' in name_upper:
                            current_manufacturer = 'TOGG'
                        elif 'HONDA' in name_upper or 'E' in name_upper:
                            current_manufacturer = 'HONDA'
                        elif 'TOYOTA' in name_upper or 'BZ4X' in name_upper:
                            current_manufacturer = 'TOYOTA'
                        elif 'MINI' in name_upper or 'COOPER SE' in name_upper:
                            current_manufacturer = 'BMW'  # Mini is owned by BMW
                        else:
                ...
                try:
                    pid_entry = PIDEntry(
                        ...
                        ecu=row.get('ECU', '').strip() if 'ECU' in row else 'BMS',
                        description=row.get('Description', '').strip() if 'Description' in row else '',
                        byte_positions=row.get('BytePositions', '').strip() if 'BytePositions' in row else 'A-B'
                    )
    ...
    def detect_vehicle_brand(self, test_pids: Optional[List[str]] = None) -> Optional[str]:
        """
        Araç markasını otomatik tespit et
        
        Args:
            test_pids: Test edilecek PID listesi (opsiyonel)
        
        Returns:
            Tespit edilen marka adı veya None
        """
        if test_pids is None:
            # Standart test PID'leri
            test_pids = [
                "Battery SOC",
                "Battery Voltage",
                "Battery Current",
        ...
        # Her marka için PID'leri test et
        for manufacturer, pids in self.manufacturer_pids.items():
            # Bu markaya özel PID'lerin varlığını kontrol et
            manufacturer_specific_count = 0
            for test_pid_name in test_pids:
                found = self.get_pid_by_name(test_pid_name, manufacturer)
                if found:
                    manufacturer_specific_count += 1
            
            # Eğer markaya özel PID'ler bulunduysa
            if manufacturer_specific_count > 0:
                self.detected_manufacturer = manufacturer
                return manufacturer
        
        return None
    
    def get_detected_brand(self) -> Optional[str]:
        """
        Tespit edilen markayı getir
        
        Returns:
            Marka adı veya None
        """
        return self.detected_manufacturer
    ...
    def get_pid_by_name(self, name: str, manufacturer: str = None) -> Optional[PIDEntry]:
        ...
        if manufacturer:
            ...
            if manufacturer_upper in ['VW', 'VOLKSWAGEN', 'AUDI', 'SEAT', 'SKODA']:
                manufacturer = 'AUDI' if 'AUDI' in manufacturer_upper else 'VW/AUDI'
                ...
                manufacturer = 'MG'
            elif manufacturer_upper in ['NISSAN', 'RENAULT']:
    ...
    def get_statistics(self) -> Dict[str, int]:
        ...
        return {
            ...
            'detected_brand': self.detected_manufacturer or 'Not Detected'
        }
